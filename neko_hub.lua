...
-- =======================================
Library.Scheme.AccentColor     = Color3.fromRGB(220, 90, 170)
Library.Scheme.BackgroundColor = Color3.fromRGB(20, 12, 18)
Library.Scheme.MainColor       = Color3.fromRGB(65, 35, 55)
Library.Scheme.OutlineColor    = Color3.fromRGB(140, 60, 110)
Library.Scheme.FontColor       = Color3.fromRGB(255, 210, 240)
-- =======================================
-- FALLENS TOGGLE MENU
local function CreateFallensToggleMenu(IconId)
    ...
    Stroke.Color = Color3.fromRGB(140,60,110)
    Stroke.Thickness = 1
    ...
    Glow.Color = Color3.fromRGB(220,70,170)
    Glow.Thickness = 5
    ...
    if Icon then
        ...
        local Image = Instance.new("ImageLabel")
        Image.BackgroundTransparency = 1
        Image.Image = "rbxassetid://" .. tostring(IconId)
        Image.Size = UDim2.fromScale(1,1)
        Image.Parent = IconFrame
    end
    ...
    MainButton.MouseEnter:Connect(function()
        game:GetService("TweenService"):Create(
            ...
            TweenInfo.new(.15),
            {
                BackgroundColor3 = Color3.fromRGB(35,18,30)
            }
        ):Play()
    end)
...
local Watermark = Library:AddDraggableLabel("NEKOHUB")
local FPSOverlayEnabled = true
...
local ParryBox = Tabs.Player:AddRightGroupbox("Parry", "swords")
...
local UISettingsBox = Tabs.Visual:AddRightGroupbox("UI Settings", "gear")
-- =============== PLAYER =================
local Players = game:GetService("Players")
local RunService = game:GetService("RunService")
...
local VaultEvent = ReplicatedStorage
    :WaitForChild("Remotes")
    :WaitForChild("Window")
    :WaitForChild("VaultEvent")
local VaultCompleteEvent = ReplicatedStorage
    :WaitForChild("Remotes")
    :WaitForChild("Window")
    :WaitForChild("VaultCompleteEvent")
...
for obj in pairs(Cached.HookPoints) do
    if obj.Parent then
...
-- Models: highlight the Bottom part for better visibility
...
if generator:IsA("Model") then
    pos = generator:GetPivot().Position
elseif generator:IsA("BasePart") then
    pos = generator.Position
end
if root and pos and (pos - root.Position).Magnitude > MAX_DISTANCE() then
...
-- Generator chams (ESP.Generator toggle)
...
-- Generator percent billboard (requires chams enabled)
local billboard = generator:FindFirstChild("GenESP")
if ESP.Generator and ESPStatus.ShowGeneratorPercent then
...
local function GetFarthestGeneratorPoint(killerRoot)
    ...
    for obj in pairs(Cached.GeneratorPoints) do
        if obj.Parent then
            local dist =
...
if not Auto.Parry and not AutoSit.Enabled then return end
...
-- AUTO SIT: right-click dodge (crouch)
if AutoSit.Enabled and AutoSit.RightClickAnims[id] then
    local myRoot = getRoot()
    ...
    if myRoot and killerRoot then
        ...
        if dist >= AutoSit.MinDistance and dist <= AutoSit.MaxDistance then
            -- Crouch ON via LeftControl key press
            local VIM = game:GetService("VirtualInputManager")
            VIM:SendKeyEvent(true, Enum.KeyCode.LeftControl, false, game)
            -- Crouch OFF after 0.5s via LeftControl key release
...
-- AUTO PARRY
if Auto.Parry and KillerAnims[fullId] then
...
local SkillCheckIndexOriginal = nil
local SkillCheckEventConnections = {}
...
if Auto.SkillCheckMode ~= "Normal" then return end
...
if goal and lineObj and line == lineObj then
    return goal
end
...
if SkillCheckIndexOriginal or not (hookmetamethod and newcclosure) then return end

local ok, original = pcall(function()
    return hookmetamethod(game, "__index", newcclosure(function(self, index)
        if Auto.SkillCheck
            and Auto.SkillCheckMode == "Instant"
            and type(index) == "string"
            and index == "Rotation"
            and not checkcaller() then
            local goal = resolveGoalForLine(self)
            if goal then
                local rotation = SkillCheckRotation.Instant
                return rotation + goal.Rotation
            end
        end

        if SkillCheckIndexOriginal then
            return SkillCheckIndexOriginal(self, index)
        end
        return nil
    end))
end)
...
if ok and type(original) == "function" then
    SkillCheckIndexOriginal = original
end
end

local function restoreRotationHook()
if not SkillCheckIndexOriginal then return end

pcall(function()
    if hookmetamethod then
        hookmetamethod(game, "__index", SkillCheckIndexOriginal)
    end
end)
SkillCheckIndexOriginal = nil
end

local function fireSkillInput()
if not Auto.SkillCheck or Auto.SkillCheckMode ~= "Instant" then return end
...
for event in pairs(SkillCheckEventConnections) do
    if not seenEvents[event] or not event.Parent then
        SkillCheckEventConnections[event]:Disconnect()
        SkillCheckEventConnections[event] = nil
    end
end
end

local SkillCheckWatcher = nil

local function startSkillCheckWatcher()
if SkillCheckWatcher then return end

SkillCheckWatcher = task.spawn(function()
    while Auto.SkillCheck and Auto.SkillCheckMode == "Instant" do
    ...
    SkillCheckWatcher = nil
end)
end

local function stopSkillCheckWatcher()
for event in pairs(SkillCheckEventConnections) do
    SkillCheckEventConnections[event]:Disconnect()
    SkillCheckEventConnections[event] = nil
end
...
local CSTag = game:GetService("CollectionService")

local function cacheObject(obj)
if obj.Name == "Generator" or CSTag:HasTag(obj, "Generator") then
Cached.Generators[obj] = true
elseif obj.Name == "Window" and obj:IsA("Model") then
Cached.Windows[obj] = true
elseif obj:IsA("BasePart") and (CSTag:HasTag(obj, "VaultPoint") and obj.Name == "VaultTrigger") then
    local win = obj.Parent
    if win and win:IsA("Model") then
        Cached.Windows[win] = true
    end
elseif obj.Name == "Pallet" or obj.Name == "Palletwrong" or CSTag:HasTag(obj, "pallet") then
Cached.Pallets[obj] = true
elseif obj.Name == "HookPoint" and obj:IsA("BasePart") then
Cached.HookPoints[obj] = true
elseif obj:IsA("BasePart") and string.match(obj.Name, "^GeneratorPoint%d+$") then
Cached.GeneratorPoints[obj] = true
end
end
...
Cached.HookPoints[obj] = nil
Cached.GeneratorPoints[obj] = nil
end
...
local function createEmoteButton()
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        -- flash efek
        stroke.Color = Color3.fromRGB(220, 90, 170)
        task.delay(0.3, function()
            stroke.Color = Color3.fromRGB(255, 255, 255)
        end)
    end)
...
RunService.RenderStepped:Connect(function()
    ...
    -- CROSSHAIR: tetap setiap frame (butuh smooth)
    drawCrosshair()
end)
...
if Auto.Parry or AutoSit.Enabled then
for _, p in pairs(Players:GetPlayers()) do
if p ~= LocalPlayer and p.Character and p.Team and p.Team.Name == "Killer" then
hookKiller(p.Character)
end
...
ESPStatusBox:AddCheckbox("ShowGeneratorPercent", {
    Text = "Show Generator Percent",
    Default = true,
    Callback = function(v)
        ESPStatus.ShowGeneratorPercent = v
end
})
...
AbilityTab:AddCheckbox("Skill", {
    ...
    Callback = function(v)
        ...
        if v then
            ...
            if Auto.SkillCheckMode == "Instant" then
                restoreRotationHook()
                stopSkillCheckWatcher()
            end
        end
    end
})

AbilityTab:AddDropdown("SkillCheckMode", {
...
ParryBox:AddCheckbox("AutoSit", {
    Text = "Auto Sit (Right Click Dodge)",
    Default = false,
    Callback = function(v)
        AutoSit.Enabled = v
    end
...
ParryBox:AddSlider("AutoSitMinDist", {
    Text = "Sit Min Distance",
    Default = 5,
    Min = 1,
    Max = 15,
    Rounding = 0,
    Callback = function(v)
        AutoSit.MinDistance = v
    end
})

ParryBox:AddSlider("AutoSitMaxDist", {
    Text = "Sit Max Distance",
    Default = 15,
    Min = 5,
    Max = 30,
    Rounding = 0,
    Callback = function(v)
        AutoSit.MaxDistance = v
    end
})
...
UISettingsBox:AddToggle("FPSOverlay", {
    Text = "FPS Overlay",
    Default = true,
    Callback = function(v)
        FPSOverlayEnabled = v
...
SaveManager:BuildConfigSection(Tabs.Visual)
